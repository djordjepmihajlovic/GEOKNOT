...
def compute_trivalent_feynman_diagram(ab_propagator, tri_propagator, samples):
    ...
    feynman_data = np.zeros((samples, 3)) # [[0, 1, 2, 3], [0, 2, 1, 3]]
    N = 152
        ...
        for i in range(0, N):
            for j in range(0, N):
                if i>j:
                    for k in range(0, N):
                        if j>k:
                            feynman_data[idy][0] += tri_propagator[idy, i, j, k]
                            for l in range(0, N):
                                if k>l:
                                    for m in range(0, N):
                                        if l>m:
                                            feynman_data[idy][1] += ab_propagator[idy][i, j]*tri_propagator[idy, k, l, m]
                                            feynman_data[idy][2] += ab_propagator[idy][i, k]*tri_propagator[idy, j, l, m]
...
def main():
    ...
    for x in knots:
        ...
        np.savetxt(f'samples/feynman_diagram_{x}_t.csv', tri2[:,0])
        np.savetxt(f'samples/feynman_diagram_{x}_t11.csv', tri2[:,1])
        np.savetxt(f'samples/feynman_diagram_{x}_t12.csv', tri2[:,2])
